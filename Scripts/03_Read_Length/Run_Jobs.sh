...
#Now use arguments in the PBS script call:
	qsub -N ${Job_Name}'_'${Sample_ID} -P wax-es -l h_rt=${TIME_LIMIT} Read_Length.qsub ${Sample_ID} ${Dataset_DIR} ${Sample_Labels_DIR} ${OUTPUT_DIR}
#Printing the qsub command which submits the job
	echo "qsub -N ${Job_Name}'_'${Sample_ID} -P wax-es -l h_rt=${TIME_LIMIT} Read_Length.qsub ${Sample_ID} ${Dataset_DIR} ${Sample_Labels_DIR} ${OUTPUT_DIR}"
#---------------------------
