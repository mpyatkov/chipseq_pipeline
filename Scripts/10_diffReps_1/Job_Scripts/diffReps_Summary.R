...
#Rscript diffReps_Summary.R diffReps_Female_Control.vs.Male_Treatment.annotated.peaks Female_Control Male_Treatment diffReps_1 Peaks_Filtered_Summary 1

DEBUG <- FALSE

Output_File <- args[1]                                # diffReps_Female_Control.vs.Male_Treatment.annotated.peaks
S1_diff <- args[2]                                    # Female_Control
S2_diff <- args[3]                                    # Male_Treatment
Peak_Caller <- args[4]                                # diffReps_1
output_dir <- args[5]                                 # Peaks_Filtered_Summary
log2FC_cutoff <- as.numeric(args[6])                  # 1

if (DEBUG){
  ## cp /projectnb/wax-dk/max/G156_H3K27ac/Scripts/10_diffReps_1/Output_diffReps_1/Overlap_Output/diffReps_Female_Control.vs.Male_Treatment.annotated.peaks ./r_refactoring
  ## cp /projectnb/wax-dk/max/G156_H3K27ac/Scripts/10_diffReps_1/Output_diffReps_1/diffReps_Female_Control.vs.Male_Treatment.hotspot ./r_refactoring
  Output_File <- "diffReps_Female_Control.vs.Male_Treatment.annotated.peaks"
  S1_diff <- "Female_Control"
  S2_diff <- "Male_Treatment"
  Peak_Caller <- "diffReps_1"
  output_dir <- "Peaks_Filtered_Summary"
  log2FC_cutoff <- 1
}
...
if ( grepl("annotated", Output_File) ) {
  #Do nothing
} else {
...
if ( length(list.files(pattern = "annotated")) >= 1)  {
  #Do nothing
} else {
...
#system(paste0("mkdir -p ",UCSC_Files_dir))
...
S2_real_delta_label <- paste0("1.",S1_diff,"_","sites (",nrow(S2_real_delta),")")

if ( dim(S2_real_delta)[1] == 0)  {
...
S2_marginal_delta_label <- paste0("2.","Control_Marginal","_","sites (",nrow(S2_marginal_delta),")")

if ( dim(S2_marginal_delta)[1] == 0)  {
...
S1_marginal_delta_label <- paste0("4.","Treatment_Marginal","_","sites (",nrow(S1_marginal_delta),")")

if ( dim(S1_marginal_delta)[1] == 0)  {
...
S1_real_delta_label <- paste0("5.",S2_diff,"_","sites (",nrow(S1_real_delta),")")

if ( dim(S1_real_delta)[1] == 0)  {
