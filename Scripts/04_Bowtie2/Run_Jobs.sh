...
#This script would be used to run Bowtie2 in parallel for different FASTQ files
#Way to run script:
#Usage: ./Bowtie2.sh
#Example: 
#./Bowtie2.sh 
...
#Now use arguments in the PBS script call:
	qsub -N ${Job_Name}'_'${Sample_ID} -P wax-es -l h_rt=${TIME_LIMIT} Bowtie2.qsub ${Sample_ID} ${Dataset_DIR} ${Sample_Labels_DIR} ${Bowtie2Index_DIR}
#Printing the qsub command which submits the job
	echo "qsub -N ${Job_Name}'_'${Sample_ID} -P wax-es -l h_rt=${TIME_LIMIT} Bowtie2.qsub ${Sample_ID} ${Dataset_DIR} ${Sample_Labels_DIR} ${Bowtie2Index_DIR}"
#---------------------------
